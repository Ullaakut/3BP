...
func ProcessBodies(bodies []Body, gravity, speed float64) []Body {
	...
	for j := 0; j < int(speed); j++ {
		...
		bodies[0].Position = bodies[0].Position.add(bodies[0].Velocity.mul(interval / bodies[0].Mass))
		bodies[1].Position = bodies[1].Position.add(bodies[1].Velocity.mul(interval / bodies[1].Mass))
		bodies[2].Position = bodies[2].Position.add(bodies[2].Velocity.mul(interval / bodies[2].Mass))
	}
